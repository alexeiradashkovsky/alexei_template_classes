...
namespace alexei_prog_snob {
    ...
    public:
        typedef std::shared_ptr<T> (*creat_t)();
    ...
    template<class T>
    dll_loader_template<T>::~dll_loader_template() {
        std::for_each(m_dll_libs.begin(), m_dll_libs.end(), 
        [](std::pair<std::string, void*>& _dll_lib) { dlclose(_dll_lib.second); } );
    }
